 ...
 */
@ConfigurationProperties(prefix = "bremersee.exception-resolver")
@Data
public class ApiExceptionResolverProperties {
  ...
  public ApiExceptionResolverProperties() {
  ...
  private boolean matches(final Throwable throwable, final String exceptionClassName) {
    ...
    if (exceptionClassName.endsWith(".*")) {
      final String packageName = exceptionClassName.substring(0, exceptionClassName.length() - 1);
  ...
  public static class ExceptionMappingConfig {
    ...
    @Getter
    @Setter
    private boolean includeHandlerClass = true;

    @Getter
    @Setter
    private boolean includeHandlerMethod = true;

    @Setter
    private boolean includeHandlerMethodParameterTypes = true;
